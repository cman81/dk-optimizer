...
if (count($argv) > 1) {
  ...
  list($threshold, $depth, $simulations) = $args;
}

// get random seeds for each simulation
if (PHP_OS == 'WINNT') {
  echo 'Enter a line of random text (mash your keyboard): ';
  $line = stream_get_line(STDIN, 1024, PHP_EOL);
} else {
  $line = readline('Enter a line of random text (mash your keyboard): ');
}
$seeds = array($line);
for ($i = 0; $i < $simulations; $i++) {
  $seeds[] = sha1(end($seeds));
  echo "Seed added: " . end($seeds) . "\n";
}
...
for ($j = 0; $j < $simulations; $j++) {
  ...
  while ($i < $threshold) {
    ...
    foreach ($players as $value) {
      $current_players[sha1(mt_rand(10000, 90000) . $seeds[$j])] = $value;
    }
    ...
    if ($current_team['total_points'] > $best_team['total_points']) {
      ...
      $i = round($i / 2);
      echo '+';
    } elseif ($current_team['total_salary'] > $limits['salary'] - 500) {
      $i++;
    }
